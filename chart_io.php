...
if(@$_SESSION["user_name"])
 {
  $user_id=@$_SESSION['user_id'];
    ...
    $query1=mysqli_query($connect,"SELECT * FROM event where  event_status='pending' ");
    $rows1 =  mysqli_num_rows($query1);

    $query2=mysqli_query($connect,"SELECT * FROM event where event_status='Approved' ");
...
function drawChart() {
  ...
  var options = {'title':'Events Posted by Faculty', 'width':550, 'height':400};
